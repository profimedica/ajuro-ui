...
export class AjuroUIColumn {
    @ContentChildren(AjuroUIColumn) public childColumns: QueryList<AjuroUIColumn>;
    ...
    private getChildColDefs(childColumns: QueryList<AjuroUIColumn>) {
        return childColumns
            // necessary because of https://github.com/angular/angular/issues/10098
            .filter(column => !column.hasChildColumns())
            .map((column: AjuroUIColumn) => {
                return column.toColDef();
            });
    };

    private createColDefFromGridColumn(from: AjuroUIColumn): ColDef {
        let colDef: ColDef = {};
